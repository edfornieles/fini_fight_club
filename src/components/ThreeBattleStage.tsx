...
import type { AnimationState, Fini, Team } from "../game/types";
import { FAMILY_COLOR } from "./familyColors";
import { FiniAvatar, moodFromHp, type FiniMood } from "./FiniAvatar";
...
export function ThreeBattleStage(props: {
  ...
  return (
    <div
      className="relative aspect-[16/9] w-full overflow-hidden rounded-3xl border-2 border-white shadow-puff"
      style={{ background: `linear-gradient(110deg, ${tint(colorA)} 0 49%, ${tint(colorB)} 51% 100%)` }}
    >
      {/* jagged lightning divider */}
      <div
        className="absolute inset-y-0 left-1/2 -translate-x-1/2 w-[8%]"
        style={{
          background: "#fff",
          clipPath:
            "polygon(40% 0, 60% 0, 48% 32%, 66% 32%, 38% 100%, 50% 52%, 34% 52%)",
          filter: "drop-shadow(0 0 6px rgba(255,255,255,0.8))",
        }}
      />

      <Fighter fini={a} anim={animA} side="left" />
      <Fighter fini={b} anim={animB} side="right" />
    </div>
  );
}
...
function Fighter({
  ...
  if (!fini) return null;
  const mood: FiniMood = moodFromHp(
    fini.maxHealth ? fini.currentHealth / fini.maxHealth : 1,
    fini.fainted,
  );
  return (
    <div
      ...
      style={animStyle(anim, side)}
    >
      <FiniAvatar family={fini.family} size={120} mood={mood} wobble={anim === "idle"} />
      <span className="chip bg-white/85 text-ink text-[11px] font-bold px-2 shadow-sm">
        {fini.name}
      </span>
    </div>
  );
...
function pickActive(team?: Team, activeId?: string): Fini | undefined {
...
function animStyle(anim: AnimationState | undefined, side: "left" | "right"): CSSProperties {
  const toCenter = side === "left" ? 28 : -28;
  const base: CSSProperties = { transition: "transform 0.22s cubic-bezier(0.34,1.56,0.64,1)" };
  switch (anim) {
    case "attack":
