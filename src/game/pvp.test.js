...
import { applyEloResult, expectedScore, leaderboard, makeSeedSnapshots, nextRating, pickOpponent, snapshotFromTeam, teamFromSnapshot, } from "./pvp";
...
describe("pvp opponent selection", () => {
    ...
    it("favors opponents near the player's rating", () => {
        ...
        const closest = [...pool].sort((a, b) => Math.abs(a.rating - 1000) - Math.abs(b.rating - 1000))[0];
...
describe("pvp leaderboard", () => {
    it("sorts by rating desc and marks the player", () => {
        const pool = makeSeedSnapshots(createRng(5));
