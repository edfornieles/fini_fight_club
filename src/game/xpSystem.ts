...
export function awardXP(fini: Fini, amount: number): Fini {
  if (amount <= 0) return fini;
 ...
 */
export function checkLevelUp(fini: Fini): {
  fini: Fini;
  records: LevelUpRecord[];
} {
  let cur = { ...fini };
...
export function computeXPAwards(args: {
  ...
  const awardMap = new Map<string, XPAward>();

  const ensure = (id: string): XPAward => {
    if (!awardMap.has(id)) {
...
export function applyBattleXPAwards(args: {
  awards: XPAward[];
  finis: Fini[];
}): { finis: Fini[]; levelUps: LevelUpRecord[] } {
