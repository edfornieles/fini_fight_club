...
export async function loadGhostTeams() {
    if (_cache)
        return _cache;
    if (_loading)
        return _loading;
    _loading = fetch("/data/ghostTeams.json")
        .then(r => {
        if (!r.ok)
            throw new Error(`ghost teams not found (${r.status}) — run scripts/seed-ghost-teams.mjs`);
        return r.json();
    })
 ...
 */
export function synthFini(tokenId, powerScale = 1) {
...
export function ghostTeamPower(tokenIds) {
 ...
 */
export async function pickGhostOpponent(yourPower, matchBand = 0.15) {
    const file = await loadGhostTeams();
    const teams = file.teams;
    if (teams.length === 0)
        throw new Error("ghost teams empty");
    // Power-band filter
    const min = yourPower * (1 - matchBand);
    const max = yourPower * (1 + matchBand);
    const scored = teams.map(t => ({ ...t, power: ghostTeamPower(t.tokenIds) }));
