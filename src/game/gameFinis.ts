...
import type { CoinFamily, Fini, FiniFrequency, FiniTraits } from "./types";
...
// ─────────────────────────────────────────────────────────────────────────
// Rarity tables (rolling)
//   Rarer frequency → stronger stat budget (via traitsToStats), but the
//   spread is modest so a lucky roll is an edge, not an auto-win.
// ─────────────────────────────────────────────────────────────────────────

type Weighted<T> = { value: T; weight: number };

const FREQUENCY_WEIGHTS: Weighted<FiniFrequency>[] = [
...
function weightedPick<T>(rng: RNG, table: Weighted<T>[]): T {
...
export type GameFiniRoll = {
  /** Synthetic id, always >= GAME_FINI_ID_BASE. */
  gameId: number;
  traits: FiniTraits;
  fini: Fini;
};
 ...
 */
export function rollGameFini(seed: number): GameFiniRoll {
  ...
  const family: CoinFamily = rng.pick(ALL_COIN_FAMILIES);
  ...
  const mythical =
    !special && rng.chance(MYTHICAL_ROLL_RATE) ? rng.pick(MYTHICAL_NAMES) : undefined;

  const traits: FiniTraits = {
    tokenId: gameId,
    family,
    frequency,
    clan,
...
export function rollPack(seed: number, n: number): GameFiniRoll[] {
  const out: GameFiniRoll[] = [];
  for (let i = 0; i < n; i++) {
...
export function isGameFini(fini: Pick<Fini, "id">): boolean {
