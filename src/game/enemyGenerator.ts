...
const FAMILY_NAME_POOL: Record<CoinFamily, string[]> = {
...
function nameFor(family: CoinFamily, rng: RNG): string {
  const pool = FAMILY_NAME_POOL[family];
  return rng.pick(pool);
}

function statsForStage(stage: number, rng: RNG): {
  strength: number;
  maxHealth: number;
  speed: number;
  defense: number;
...
function packNameToFamily(name?: string): CoinFamily | null {
  if (!name) return null;
  const lower = name.toLowerCase();
  if (lower.includes("shiba") || lower.includes("dog") || lower.includes("meme"))
    return "DOGE";
  if (lower.includes("gas") || lower.includes("beacon")) return "ETH";
  if (lower.includes("liquidity") || lower.includes("hound")) return "UNI";
  if (lower.includes("whale") || lower.includes("orange")) return "BTC";
  if (lower.includes("validator")) return "BNB";
  if (lower.includes("oracle")) return "LINK";
  if (lower.includes("snow") || lower.includes("avalanche")) return "AVAX";
  if (lower.includes("wick")) return "SOL";
  if (lower.includes("stable")) return "BNB";
  return null;
}
