...
function baseConfig(over: Partial<BattleConfig> = {}): BattleConfig {
  return {
    mode: "FREE",
    battleWindow: "1h",
    maxRounds: 30,
...
function totalDamageBy(team: Team, result: ReturnType<typeof runBattle>): number {
  const ids = new Set(team.finis.map((f) => f.id));
  let sum = 0;
  for (const ev of result.events) {
    if (ev.type === "ATTACK" && ids.has(ev.attackerId)) sum += ev.damage;
  }
  return sum;
}

describe("market read", () => {
  const signals: MarketSignalMap = getManualTestSignals();
  ...
  it("a correct read increases the calling team's damage output", () => {
    ...
    expect(totalDamageBy(team("A"), withRead)).toBeGreaterThan(
      totalDamageBy(team("A"), without),
    );
  });
  ...
  it("a wrong read grants no damage bonus", () => {
    ...
    expect(totalDamageBy(team("A"), wrongRead)).toBe(
      totalDamageBy(team("A"), without),
    );
  });
});
