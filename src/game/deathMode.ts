...
import {
  deathModeClosingLines,
  deathModeOpeningLines,
  deathModeStakeLine,
} from "./battleLogText";
...
export function makeEmptyDeathModeConfig(): DeathModeConfig {
  return {
    enabled: false,
    simulatedOnly: true,
    stakes: {
...
export type DeathModeReadiness =
  | { ok: true }
  | {
      ok: false;
      reason:
...
export function buildDeathModeOpeningEvents(args: {
  ...
  if (!stakeA || !stakeB) return events;
...
export function resolveSimulatedDeathMode(args: {
  ...
  teamA: Team;
  teamB: Team;
}): { deathModeResult: DeathModeResult; events: BattleEvent[] } {
  const { result, config, teamA, teamB } = args;

  const winningStake: DeathModeStake =
    result.winner === "teamA" ? config.stakes.teamA : config.stakes.teamB;
  const losingStake: DeathModeStake =
    result.winner === "teamA" ? config.stakes.teamB : config.stakes.teamA;
  ...
  // libraries here.
  mockOwnershipLedger.transfer(
    losingStake.finiId,
    losingStake.playerId,
    winningStake.playerId,
  );

  const wonFini =
    (result.winner === "teamA" ? teamB : teamA).finis.find(
      (f) => f.id === losingStake.finiId,
    ) ?? null;
  const lostFini =
    (result.winner === "teamA" ? teamA : teamB).finis.find(
      (f) => f.id === winningStake.finiId,
    ) ?? null;

  const dmr: DeathModeResult = {
  ...
  events.push({
    ...
    message: `Simulated transfer complete. ${
      wonFini?.family ?? "?"
    } Fini #${wonFini?.tokenId ?? wonFini?.id ?? dmr.wonFiniId} now belongs to ${
      dmr.winnerPlayerId
    }.`,
  });
  for (const closing of deathModeClosingLines(
    wonFini?.family ?? "BTC",
    wonFini?.tokenId ?? wonFini?.id ?? dmr.wonFiniId,
  )) {
    events.push({
      type: "DEATH_MODE_TRANSFER",
      winnerPlayerId: dmr.winnerPlayerId,
...
export async function realDeathModeTransfer(_args: {
  ...
  throw new Error(
    "Real Death Mode NFT transfer is not implemented. " +
