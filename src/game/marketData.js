...
const ID_TO_FAMILY = Object.fromEntries(Object.entries(COINGECKO_IDS).map(([fam, id]) => [
    id,
    fam,
]));
 ...
 */
function percentForWindow(window, change1h, change24h) {
    switch (window) {
        case "5m":
 ...
 */
function volatilityFromRange(high, low, current) {
    if (!high || !low || !current || current <= 0)
        return 0.4;
 ...
 * offline.
 */
export async function fetchLiveMarketSignals(window, signal) {
    ...
    for (const fam of ALL_COIN_FAMILIES) {
        ...
        const volatility = volatilityFromRange(row.high_24h, row.low_24h, row.current_price);
