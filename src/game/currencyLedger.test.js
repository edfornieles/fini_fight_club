...
function totalInSystem(ledger, players, escrows) {
...
describe("MockCurrencyLedger — conservation invariant", () => {
    it("a full deposit → payout → house-sweep conserves money exactly", () => {
        ...
        for (const p of players)
            expect(l.escrowDeposit("pot", p, 25)).toBe(true);
    ...
    it("randomised deposits/payouts never create or destroy money", () => {
        ...
        for (const amt of seq) {
            ...
            const e = i % 2 === 0 ? "e1" : "e2";
            if (l.getBalance(p) >= amt)
                l.escrowDeposit(e, p, amt);
            // pay some back to a different player + sweep a bit to house
            if (l.escrowBalance(e) >= 2)
                l.payout(e, players[(i + 1) % players.length], 1);
            if (l.escrowBalance(e) >= 1)
                l.houseCollect(e, 1);
            i++;
        }
