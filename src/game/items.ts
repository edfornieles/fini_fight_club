...
/**
 * Items are equippable modifiers that live in the player's inventory.
 * One item can be equipped per Fini (one slot, mirroring the existing
 * battles.pixelsolve.net design).
 *
 * Each item provides flat stat bonuses. Some items can also tag a Fini
 * with a buff that the battle engine reads (extending later).
 */

export type ItemKind = "WEAPON" | "ARMOR" | "TRINKET" | "POTION";
...
export type Item = {
  ...
  /** Optional flavor tag the battle engine could react to in future. */
  tag?: string;
};
...
export function pickItemForShop(rng: () => number): Item {
 ...
 * into runBattle().
 */
export function applyItemToFini(fini: Fini, item: Item | null): Fini {
  if (!item) return { ...fini };
...
export function describeItemDelta(item: Item): string[] {
  const out: string[] = [];
  const b = item.bonuses;
  if (b.strength) out.push(`+${b.strength} ATK`);
  if (b.maxHealth) out.push(`+${b.maxHealth} HP`);
  if (b.speed) out.push(`+${b.speed} SPD`);
  if (b.defense) out.push(`+${b.defense} DEF`);
  if (b.volatilityAffinity) out.push(`+${b.volatilityAffinity} VOL`);
  if (b.cuteness) out.push(`+${b.cuteness} CUTE`);
  return out;
}
