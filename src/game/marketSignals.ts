...
const WINDOW_CONFIG: Record<
  BattleWindow,
  { magnitude: number; volatility: number }
> = {
...
function directionFromPercent(percent: number): MarketDirection {
  if (percent > 0.4) return "up";
  if (percent < -0.4) return "down";
  return "flat";
}
...
function generateOneSignal(
  ...
  const familyMult: Record<CoinFamily, number> = {
    BTC: 0.6,
    ETH: 0.9,
    SOL: 1.3,
    DOGE: 1.6,
  ...
  const volatility = Math.min(
    1,
    Math.max(0, cfg.volatility * (0.6 + rng.next() * 0.8)),
  );
  return normalizeMarketSignal(percentChange, volatility, family);
}

export function generateMockMarketSignals(
  window: BattleWindow,
  seed?: number,
): MarketSignalMap {
 ...
 */
export function getManualTestSignals(): MarketSignalMap {
  const manual: Record<CoinFamily, { percent: number; volatility: number }> = {
...
export function strongestFamily(signals: MarketSignalMap): CoinFamily {
  let best: CoinFamily = ALL_COIN_FAMILIES[0]!;
