...
export type FunnelTierId = "PRACTICE" | "MICRO" | "BRONZE" | "SILVER";

export type FunnelTier = {
  id: FunnelTierId;
  label: string;
  /** Entry fee in whole dollars. 0 = free. */
  entryFee: number;
  /** Pot the house guarantees the league reaches (CAC). 0 = no seed. */
  guaranteedPot: number;
  /** Unlocks after the player has completed this many leagues (gating). */
  unlockAfterPlays: number;
  /** House rake as basis points (1000 = 10%). Free tier takes none. */
  rakeBps: number;
};
 ...
 */
export const FUNNEL_LADDER: FunnelTier[] = [
...
export function tierById(id: FunnelTierId): FunnelTier {
  const t = FUNNEL_LADDER.find((x) => x.id === id);
  if (!t) throw new Error(`unknown funnel tier ${id}`);
  return t;
}

/** Which tiers a player can enter given how many leagues they've completed. */
export function unlockedTiers(playsCompleted: number): FunnelTier[] {
  return FUNNEL_LADDER.filter((t) => playsCompleted >= t.unlockAfterPlays);
}

export function isTierUnlocked(id: FunnelTierId, playsCompleted: number): boolean {
...
export function rollCost(n: number, freeRollsRemaining: number): number {
 ...
 */
export function breakEvenEntrants(tier: FunnelTier): number {
  if (tier.entryFee === 0) return Infinity; // free tier never makes money directly
  for (let n = 1; n <= 64; n++) {
    if (leagueHouseEdge({ tier, realEntrants: n }) >= 0) return n;
  }
  return Infinity;
}
