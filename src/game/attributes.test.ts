...
import {
  traitsToStats,
  familyMatchup,
  familyMatchupWithPerks,
  validateTeamSpecials,
  countSpecialFinis,
  SPECIAL_PERKS,
} from "./attributes";
import type { FiniTraits } from "./types";
import { ALL_COIN_FAMILIES } from "./types";

// ─── fixtures ──────────────────────────────────────────────────────────────

const BASE_TRAITS: FiniTraits = {
  tokenId: 1,
  family: "ETH",
  frequency: "Hourly",
  clan: "Blades",
...
describe("frequency rarity budget", () => {
  const frequencies = ["Hourly", "Daily", "Twice-Daily", "Weekly", "Monthly"] as const;
...
describe("team special validation", () => {
  ...
  const withSpecial = { specialPerk: "DIAMOND_HANDS" as const, mythicalPerk: undefined };
  const withMythical = { specialPerk: undefined, mythicalPerk: "GENESIS_BLOCK" as const };
  ...
  it("cycle is consistent: if A beats B, then B loses to A", () => {
    for (const a of ALL_COIN_FAMILIES) {
      for (const b of ALL_COIN_FAMILIES) {
        ...
        if (ab === 1.10) expect(ba).toBe(0.90);
        if (ab === 0.90) expect(ba).toBe(1.10);
        if (ab === 1.00) expect(ba).toBe(1.00);
      }
    }
  });
...
describe("family fairness", () => {
  it("every family has exactly one soft counter in the cycle", () => {
    ...
    for (const family of ALL_COIN_FAMILIES) {
      const counters = ALL_COIN_FAMILIES.filter(
        opp => opp !== family && familyMatchup(opp, family) === 1.10,
      );
  ...
  it("volatility affinity is well-spread (DOGE/SOL high, BTC/XTZ low)", () => {
    ...
    const affinities = ALL_COIN_FAMILIES.map(f =>
      traitsToStats({ tokenId: 1, family: f, frequency: "Hourly", clan: "Stickies", latestDelta: 0 })
        .volatilityAffinity,
    );
