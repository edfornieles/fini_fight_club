...
function leagueWith(n, seed = 1) {
    const snaps = makeSeedSnapshots(createRng(seed)).slice(0, n);
    const entries = snaps.map((s) => ({ playerId: s.id, snapshot: s }));
...
describe("knockout bracket", () => {
    ...
    it("is fully deterministic for the same entries + seed", () => {
        ...
        expect(a.result.standings.map((s) => s.playerId)).toEqual(b.result.standings.map((s) => s.playerId));
    });
