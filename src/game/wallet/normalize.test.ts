...
describe("metadataToOwnedFini", () => {
  it("resolves artwork + price into a display record", () => {
    ...
    expect(f.artwork.imageUrl).toContain(
      "/ggneCmg9gWZmdZHqWNKoHZeh26xSwd3lgZkeINli8MI/1.gif",
    );
    expect(f.artwork.imageUrls.length).toBeGreaterThanOrEqual(2);
    expect(f.artwork.animationUrl).toContain(
      "/ggneCmg9gWZmdZHqWNKoHZeh26xSwd3lgZkeINli8MI/1.mp4",
    );
