...
import type { OwnedFini, OwnershipProvider, OwnershipSnapshot } from "./types";
...
const FREQUENCIES: FiniFrequency[] = [
  "Hourly",
  "Daily",
  "Twice-Daily",
  "Weekly",
...
function mockTraits(wallet: string, tokenId: number): FiniTraits {
...
export class MockOwnershipProvider implements OwnershipProvider {
  ...
  constructor(private readonly countPerWallet = 6) {}

  async getOwnedTokenIds(wallet: string): Promise<number[]> {
  ...
  async getFini(tokenId: number): Promise<OwnedFini> {
  ...
  async getRoster(wallet: string): Promise<OwnedFini[]> {
    const addr = normalizeAddress(wallet) ?? wallet;
...
async function loadSnapshot(url: string): Promise<OwnershipSnapshot> {
  const res = await fetch(url);
  if (!res.ok) throw new Error(`snapshot HTTP ${res.status}`);
  return (await res.json()) as OwnershipSnapshot;
}

export class SnapshotOwnershipProvider implements OwnershipProvider {
  readonly id = "snapshot" as const;
  private snapshot: OwnershipSnapshot | null;
  private readonly url: string;

  constructor(opts?: { snapshot?: OwnershipSnapshot; url?: string }) {
    this.snapshot = opts?.snapshot ?? null;
    this.url = opts?.url ?? defaultSnapshotUrl();
  }

  private async ensure(): Promise<OwnershipSnapshot> {
    if (!this.snapshot) this.snapshot = await loadSnapshot(this.url);
...
export class LiveOwnershipProvider implements OwnershipProvider {
  readonly id = "live" as const;
  private readonly snap: SnapshotOwnershipProvider;
  ...
  async getOwnedTokenIds(wallet: string): Promise<number[]> {
    ...
    const checks = await Promise.all(
      candidates.map(async (id) => ((await ownsToken(wallet, id)) ? id : -1)),
    );
...
let snapshotCache: OwnershipSnapshot | null = null;

/** Load + cache the full ownership snapshot (for collection-wide lookups). */
export async function loadOwnershipSnapshot(
  url: string = defaultSnapshotUrl(),
...
export function defaultSnapshotUrl(): string {
