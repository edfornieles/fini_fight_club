...
export function ownedFiniToBattleFini(owned) {
    const stats = traitsToStats(owned.traits);
    // Prefer the animated mp4 for an in-battle model hook; fall back to the gif.
    const modelUrl = owned.artwork.animationUrl ?? owned.artwork.imageUrl ?? undefined;
    return {
...
export function ownedFinisToBattleFinis(owned) {
