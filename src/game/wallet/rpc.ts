...
function decodeAddressWord(hex: string): string | null {
...
async function ethCall(data: string): Promise<string> {
  ...
  throw new Error(
    `all public RPCs failed: ${lastErr instanceof Error ? lastErr.message : String(lastErr)}`,
  );
}
...
export async function balanceOf(wallet: string): Promise<number> {
  ...
  if (!addr) throw new Error("invalid wallet address");
...
export async function ownsToken(wallet: string, tokenId: number): Promise<boolean> {
