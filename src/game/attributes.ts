...
// ─────────────────────────────────────────────────────────────────────────
// 1. Family base blocks
//    Budget target: ~30 points (str+hp+spd+def) for most families.
//    BTC is intentionally higher (~38) because all-in tank; that spread
//    is balanced by the counter-triangle and market volatility affinity.
// ─────────────────────────────────────────────────────────────────────────
...
type FamilyBase = {
  ...
  archetype: string; // for documentation / UI tooltip
};

const FAMILY_BASE: Record<CoinFamily, FamilyBase> = {
...
const FREQ_MULT: Record<FiniFrequency, number> = {
  Hourly: 1.00,
  Daily: 1.07,
  "Twice-Daily": 1.14,
  Weekly: 1.21,
...
// ─────────────────────────────────────────────────────────────────────────
// 3. Clan → passive override + stat lean
//    146 clans hash-map to 10 passive buckets. Each bucket gives a small
//    lean (before frequency scaling so the lean itself also scales).
//    The clan's passive may differ from the family default — owning a
//    DOGE with a COMPOUND clan gives a unique DOGE that compounds instead
//    of meme-spiking, changing how you play it.
// ─────────────────────────────────────────────────────────────────────────
...
function clanLean(clan: string): ClanLean {
...
function tokenJitter(
  tokenId: number,
): { stat: "str" | "hp" | "spd" | "def" | "none"; delta: 1 } {
  // Knuth multiplicative hash, pick one of 5 buckets (4 stats + no-op)
  const h = Math.imul(tokenId, 2654435769) >>> 0;
  const bucket = h % 5;
  const stats = ["str", "hp", "spd", "def", "none"] as const;
  return { stat: stats[bucket], delta: 1 };
}

// ─────────────────────────────────────────────────────────────────────────
// 5. Special perks  (48 trait values → 24 perk mechanics)
//    Most Finis have no special. One special per team is enforced at
//    team-selection time (see validateTeamSpecials below).
//    Stat mods are small (2–4 points); the MECHANIC is what makes them
//    genuinely special. Battle engine reads the perk ID at runtime.
// ─────────────────────────────────────────────────────────────────────────

export type SpecialPerkDef = {
  id: SpecialPerkId;
  displayName: string;
  description: string;
  // Flat stat mods applied on top of the fully-scaled base block.
  dStr: number;
  dHp: number;
  dSpd: number;
  dDef: number;
...
export const SPECIAL_PERKS: Record<SpecialPerkId, SpecialPerkDef> = {
  DIAMOND_HANDS: {
...
const KNOWN_SPECIAL_MAP: Record<string, SpecialPerkId> = {
...
const SPECIAL_PERK_IDS = Object.keys(SPECIAL_PERKS) as SpecialPerkId[];

function resolveSpecialPerk(name: string): SpecialPerkId {
  return KNOWN_SPECIAL_MAP[name] ?? SPECIAL_PERK_IDS[hashString(name) % SPECIAL_PERK_IDS.length];
}

// ─────────────────────────────────────────────────────────────────────────
// 6. Mythical perks  (only 3 exist in 10 k tokens — legendary mechanics)
// ─────────────────────────────────────────────────────────────────────────

export type MythicalPerkDef = {
  id: MythicalPerkId;
  displayName: string;
  description: string;
  dStr: number;
...
export const MYTHICAL_PERKS: Record<MythicalPerkId, MythicalPerkDef> = {
  GENESIS_BLOCK: {
...
const MYTHICAL_PERK_IDS = Object.keys(MYTHICAL_PERKS) as MythicalPerkId[];

// Known on-chain mythical names (fill in when real names are confirmed).
const KNOWN_MYTHICAL_MAP: Record<string, MythicalPerkId> = {
  "Genesis Block": "GENESIS_BLOCK",
  "The Merge": "THE_MERGE",
  "Nakamoto": "NAKAMOTO",
};

function resolveMythicalPerk(name: string): MythicalPerkId {
...
export function traitsToStats(t: FiniTraits): BattleStatBlock {
  ...
  if (jitter.stat === "str") str += jitter.delta;
  else if (jitter.stat === "hp")  hp  += jitter.delta;
  else if (jitter.stat === "spd") spd += jitter.delta;
  else if (jitter.stat === "def") def += jitter.delta;

  // Clan may override the family's default passive.
  const passive = lean.passive;

  // Step 5: special perk
  let specialPerk: SpecialPerkId | undefined;
  if (t.special) {
  ...
  let mythicalPerk: MythicalPerkId | undefined;
  if (t.mythical) {
...
const COUNTER_CYCLE: CoinFamily[] = [
 ...
 */
export function familyMatchup(
  attacker: CoinFamily,
  defender: CoinFamily,
): number {
  if (attacker === defender) return COUNTER_NEUT;
  const atkIdx = COUNTER_CYCLE.indexOf(attacker);
  const defIdx = COUNTER_CYCLE.indexOf(defender);
  if (atkIdx === -1 || defIdx === -1) return COUNTER_NEUT;
 ...
 */
export function familyMatchupWithPerks(
  attacker: CoinFamily,
  attackerSpecialPerk: SpecialPerkId | undefined,
  defender: CoinFamily,
): number {
 ...
 */
export function countSpecialFinis(finis: { specialPerk?: SpecialPerkId; mythicalPerk?: MythicalPerkId }[]): number {
  return finis.filter(f => f.specialPerk !== undefined || f.mythicalPerk !== undefined).length;
}

export function validateTeamSpecials(finis: { specialPerk?: SpecialPerkId; mythicalPerk?: MythicalPerkId }[]): boolean {
...
export function getFamilyArchetype(family: CoinFamily): string {
  return FAMILY_BASE[family].archetype;
}

export function getFamilyDefaultPassive(family: CoinFamily): PassiveAbility {
  return FAMILY_BASE[family].passive;
}

/** Full public identity of a family — for the Explore page. */
export type FamilyInfo = {
  family: CoinFamily;
  archetype: string;
  baseStats: { strength: number; maxHealth: number; speed: number; defense: number };
  volatilityAffinity: number;
  cuteness: number;
  defaultPassive: PassiveAbility;
  /** Families this one beats (×1.10) and loses to (×0.90) when attacking. */
  beats: CoinFamily;
  losesTo: CoinFamily;
};

export function getFamilyInfo(family: CoinFamily): FamilyInfo {
...
/** What a clan name does in the system: its passive bucket + stat lean. */
export type ClanProfile = {
  clan: string;
  passive: PassiveAbility;
  statLean: { strength: number; maxHealth: number; speed: number; defense: number; volatilityAffinity: number };
};

export function getClanProfile(clan: string): ClanProfile {
