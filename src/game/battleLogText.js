...
export function familyMarketLine(signal) {
 ...
 */
export function marketTickLine(family, _direction, delta) {
...
export function attackLine(attacker, defender, damage) {
    const verbs = [
        "lunges at",
        "strikes",
        "headbutts",
...
export function faintLine(fini) {
    const tail = [
...
export function damageLine(fini, dmg, remaining) {
    return `${fini.name} takes ${dmg} damage. ${remaining} HP remaining.`;
}
export function deathModeStakeLine(fini) {
    return `${fini.family} Fini #${fini.tokenId ?? fini.id} is now exposed.`;
}
export function deathModeOpeningLines() {
    return [
...
export function deathModeClosingLines(wonFamily, wonId) {
    return [
        "Death Mode Complete.",
