...
import { buildDeathModeOpeningEvents, checkDeathModeReadiness, makeEmptyDeathModeConfig, resolveSimulatedDeathMode, } from "./deathMode";
...
describe("Death Mode", () => {
    ...
    it("cannot start without both confirmations", () => {
        ...
        if (!res.ok)
            expect(res.reason).toBe("UNCONFIRMED_B");
    });
    ...
    it("simulated resolve assigns loser's staked Fini to winner and updates ledger", () => {
        ...
        for (const f of teamA.finis)
            mockOwnershipLedger.setOwner(f.id, "player-a");
        for (const f of teamB.finis)
            mockOwnershipLedger.setOwner(f.id, "player-b");
        const cfg = makeEmptyDeathModeConfig();
        cfg.enabled = true;
        cfg.stakes.teamA = {
            playerId: teamA.playerId,
