...
const outperformBattles = [];
let idx = 0;
for (let i = 0; i < COINS.length; i++) {
    for (let j = i + 1; j < COINS.length; j++) {
        const a = COINS[i], b = COINS[j];
        const pctA = seedPct(a, b);
        const status = idx < 6 ? "live" : idx < 12 ? "upcoming" : "upcoming";
...
export const MOCK_BATTLES = [...upDownBattles, ...outperformBattles, ...specialBattles];
export function getBattlesByAsset(asset) {
...
export function computeOdds(change24h) {
    const bias = Math.min(15, Math.max(-15, change24h * 1.5));
    const pctA = Math.round(50 + bias);
    return { pctA, pctB: 100 - pctA };
}
export function liveVolume(_prices, asset) {
    const base = { BTC: 120, ETH: 90, SOL: 60, DOGE: 50, BNB: 40 };
