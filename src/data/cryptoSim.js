 ...
 */
function fairOddsForBattle(battle, initialEndsAt) {
...
const battleEndsAt = new Map();
const shortWallet = (a) => a.length >= 10 ? `${a.slice(0, 6)}…${a.slice(-4)}` : a;
export const useCryptoSim = create((set, get) => ({
    ...
    start: () => {
        ...
        for (const b of get().battles) {
            if (!battleEndsAt.has(b.id))
                battleEndsAt.set(b.id, Date.now() + b.endsInMs);
        }
        timer = setInterval(() => {
