...
export function useSiweAuth() {
  ...
  const [status, setStatus] = useState<"idle" | "signing" | "verifying" | "authed" | "error">("idle");
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      const w = (data.session?.user?.user_metadata?.wallet as string | undefined)?.toLowerCase();
      if (w) { setAuthedWallet(w); setStatus("authed"); }
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (!isConnected || !address) {
      if (authedWallet) { supabase.auth.signOut(); setAuthedWallet(null); setStatus("idle"); }
      return;
    }
    const a = address.toLowerCase();
    if (authedWallet === a) return;
