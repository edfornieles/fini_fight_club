...
export async function fetchPricesMulti() {
    ...
    for (const sym of SYMBOLS) {
        const samples = [];
        if (cgResult?.[sym]?.usd)
            samples.push({ source: "coingecko", usd: cgResult[sym].usd });
        if (coinbaseResult[sym]?.usd)
            samples.push({ source: "coinbase", usd: coinbaseResult[sym].usd });
        if (binanceResult[sym]?.usd)
            samples.push({ source: "binance", usd: binanceResult[sym].usd });
        if (samples.length === 0)
            continue;
...
async function tryCoinGecko() {
    ...
    for (const sym of SYMBOLS) {
        ...
        if (r?.usd)
            out[sym] = { usd: r.usd, usd_24h_change: r.usd_24h_change ?? 0 };
    }
    return out;
}
async function tryCoinbase() {
    ...
    await Promise.all(SYMBOLS.map(async (sym) => {
        ...
        if (!pair)
            return;
        try {
            const r = await withTimeout(fetch(`https://api.exchange.coinbase.com/products/${pair}/stats`), PROVIDER_TIMEOUT_MS);
            if (!r.ok)
                return;
...
async function tryBinance() {
    ...
    try {
        ...
        for (const row of arr) {
            const sym = Object.keys(BINANCE_PAIRS).find(s => BINANCE_PAIRS[s] === row.symbol);
            if (!sym)
                continue;
