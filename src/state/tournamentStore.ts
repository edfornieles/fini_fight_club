...
import type { AnimationState, BattleResult, Fini, Team } from "../game/types";
import {
  createBracket,
  playMatch,
  recordResult,
...
import type { LeagueEntry } from "../game/leagues";
...
export type TournyStatus = "idle" | "bracket" | "playing" | "matchOver" | "champion";

type Live = {
  team: Team; // cloned, HP mutated during playback
  activeId: string;
  anim: AnimationState;
  score: number; // faints inflicted on the opponent
};

type FloatDmg = { side: "a" | "b"; amount: number; key: number };
...
type TournamentStore = {
  ...
  liveB: Live | null;
  message: string;
  floatDmg: FloatDmg | null;
  champion: LeagueEntry | null;
  prizePool: number;

  start: (playerTeam: Team) => void;
  playNext: () => void;
  tick: () => void;
  skipMatch: () => void;
...
function buildEntrants(playerTeam: Team, seed: number): LeagueEntry[] {
  const pool = makeSeedSnapshots(createRng(seed));
  const playerEntry: LeagueEntry = {
    playerId: PLAYER_ID,
    snapshot: snapshotFromTeam({ team: playerTeam, name: "You", rating: 1000, origin: "player" }),
  };
  const cpu: LeagueEntry[] = pool.slice(0, FIELD - 1).map((snap, i) => ({
    playerId: `cpu-${i}`,
...
function freshLive(team: Team): Live {
  const cloned: Team = {
    ...team,
    finis: team.finis.map((f) => ({ ...f, currentHealth: f.maxHealth, fainted: false })) as [Fini, Fini, Fini],
  };
...
export const useTournamentStore = create<TournamentStore>((set, get) => ({
  ...
  tick: () => {
    ...
    if (!result || !liveA || !liveB || !bracket) return;

    if (eventIndex >= result.events.length) {
      // playback finished → record + advance bracket
      const match = findMatch(bracket, matchId!)!;
      if (!match.winnerId) recordResult(bracket, matchId!, result);
    ...
    let a = { ...liveA, anim: "idle" as AnimationState };
    let b = { ...liveB, anim: "idle" as AnimationState };
    let floatDmg: FloatDmg | null = null;
    ...
    if (ev.type === "ATTACK") {
      ...
      if (fini) {
        side.team = {
          ...side.team,
          finis: side.team.finis.map((f) =>
            f.id === ev.finiId ? { ...f, currentHealth: Math.max(0, ev.remainingHealth) } : f,
          ) as [Fini, Fini, Fini],
        };
      }
      ...
      side.team = {
        ...side.team,
        finis: side.team.finis.map((f) =>
          f.id === ev.finiId ? { ...f, fainted: true, currentHealth: 0 } : f,
        ) as [Fini, Fini, Fini],
      };
      if (inA) b = { ...b, score: b.score + 1 };
      else a = { ...a, score: a.score + 1 };
    }
...
export function championPrize(pool: number): number {
