...
import { createLeague, joinLeague, settleLeague, escrowIdFor, computePrizeBreakdown, TIER_BUYINS, } from "../game/leagues";
import { runBracketLeague } from "../game/bracket";
...
export const ACTIVE_TIERS = ["BRONZE", "SILVER"];
/**
...
export const useLeagueStore = create((set, get) => {
    ...
    return {
        ...
        refreshOpenLeagues: () => {
            ...
            const kept = get().leagues.filter((l) => l.status === "OPEN" || l.status === "RUNNING");
            const tiersPresent = new Set(kept.map((l) => l.config.tier));
            const fresh = ACTIVE_TIERS.filter((t) => !tiersPresent.has(t)).map((t) => seedLeague(t));
