...
interface CoinState {
  ...
  /** Optimistically spend (UI only). Use spendOnServer for real spending. */
  spend: (n: number) => void;
  /** Optimistically earn. Use the credit endpoint server-side for real credits. */
  earn:  (n: number) => void;
  /** Authoritative spend through the debit-balance edge function. */
...
if (isOnline) {
  supabase.auth.onAuthStateChange((event, session) => {
    ...
    if (event === "SIGNED_OUT") useCoinStore.setState({ balance: 0, loaded: false });
  });
}
