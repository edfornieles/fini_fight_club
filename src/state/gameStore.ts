...
import {
  ...
  loadProfile,
  saveProfile,
  updateSnapshotRating,
} from "../game/pvpStorage";
...
import {
  loadOwnedTeam,
  saveOwnedTeam,
  clearOwnedTeam as clearOwnedTeamStorage,
  type SavedOwnedTeam,
} from "../game/wallet";
import { mockOwnershipLedger } from "../game/ownership";
import { cloneTeam, rehydrateTeam } from "../game/finiFactory";
import { rollShop, type ShopState } from "../game/shop";
import { applyItemToFini, type Item } from "../game/items";
import {
...
import {
  ENCOUNTERS_PER_STAGE,
  FINAL_STAGE,
  GOLD_REWARD_PER_BOSS_WIN,
  GOLD_REWARD_PER_FIGHT_WIN,
  POST_STAGE_GOLD_REFRESH,
  ROLL_COST,
  SELL_REFUND,
  STARTING_GOLD,
  STARTING_LIVES,
  TROPHIES_PER_BOSS_WIN,
  UNIT_COST,
} from "../game/runConstants";

/**
 * Run-based meta-loop on top of the pure battle engine.
 *
 * Phase router:
 *   title    -> Start New Run -> shop
 *   shop     -> Ready -> encounter
 *   encounter -> picks one:
 *      FIGHT/BOSS_FIGHT -> battle -> result -> next stage's shop
 *      FOUND_COINS / TREASURE -> apply effect -> advance -> shop
 *      VISIT_SHOP -> shop (re-rolled, free)
 *      REST -> apply heal -> advance -> shop
 *      DEATH_MATCH -> opens DeathModeModal, then battle
 *   result   -> Continue -> shop (next stage)
 *   gameOver / victory -> Start New Run -> title
 *
 * The pure battle engine still runs unmodified. We just call it with
 * items applied to the player team and a procedurally-generated enemy
 * team.
 */
...
export type PlaybackStatus = "idle" | "preparing" | "playing" | "ended";
export type MarketMode = "MOCK" | "MANUAL" | "LIVE";
...
type GameState = {
  ...
  trophies: number;
  stage: number;
  stageProgress: number; // 0..ENCOUNTERS_PER_STAGE

  teamSlots: [TeamSlotState, TeamSlotState, TeamSlotState];
  inventory: Item[];
  ...
  // ─────────── Market meta (daily regime + read) ───────────
  dailyRegime: DailyRegime;
  /** the player's pre-battle call for this stage; null = no call made */
  marketRead: CoinFamily | null;
  /** cached LIVE snapshot so battles in a window are consistent */
  liveSignals?: MarketSignalMap;
  ...
  pvpPool: TeamSnapshot[];
  currentOpponent: TeamSnapshot | null;
  /** ELO change from the most recent ranked match, for the result screen */
  rankedDelta: number | null;

  deathMode: DeathModeConfig;
  deathConfirmInput: string;
  deathModePending: boolean; // when the modal is open and battle hasn't started
  ...
  // ─────────── Actions ───────────
  /** The player's persisted wallet team (survives reloads), if any. */
  savedOwnedTeam: SavedOwnedTeam | null;

  startNewRun: () => void;
  startRankedLadder: () => void;
  /**
   * Field wallet-owned Finis as a ranked team (skips drafting units).
   * Returns an error string if the lineup is illegal (e.g. >1 special), else null.
   */
  fieldOwnedTeam: (finis: Fini[], wallet?: string) => string | null;
  /** Forget the persisted wallet team. */
  clearOwnedTeam: () => void;
  exitToTitle: () => void;
  ...
  rollShop: () => void;
  toggleShopLock: () => void;
  buyUnit: (shopIdx: number, slotIdx: 0 | 1 | 2) => string | null;
  buyItem: (shopIdx: number, slotIdx: 0 | 1 | 2) => string | null;
  sellUnit: (slotIdx: 0 | 1 | 2) => void;
  swapTeamSlots: (a: 0 | 1 | 2, b: 0 | 1 | 2) => void;
  equipItemOnSlot: (itemId: string, slotIdx: 0 | 1 | 2) => void;
  unequipSlot: (slotIdx: 0 | 1 | 2) => void;
  readyForEncounter: () => void;

  // Encounter
  pickEncounter: (encounterId: string) => void;

  // Battle controls
  setMode: (m: BattleMode) => void;
  setBattleWindow: (w: BattleWindow) => void;
  setMarketMode: (m: MarketMode) => void;
  regenerateMarketSignals: () => void;
  setMarketRead: (family: CoinFamily | null) => void;
  refreshLiveMarket: () => Promise<void>;
  setDeathStake: (side: "teamA" | "teamB", finiId: string) => void;
  setDeathConfirmInput: (s: string) => void;
  confirmDeathMode: (side: "teamA" | "teamB") => void;
  cancelDeathMode: () => void;
...
function makeEmptyTeamSlots(): [TeamSlotState, TeamSlotState, TeamSlotState] {
  return [
...
function teamSlotsFilled(
  slots: [TeamSlotState, TeamSlotState, TeamSlotState],
): number {
 ...
 */
function buildBattleSignals(s: GameState): MarketSignalMap {
  let base: MarketSignalMap;
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  startNewRun: () =>
    set(() => {
      ...
      for (const f of []) mockOwnershipLedger.setOwner(f, "player-a");
  ...
  toggleShopLock: () =>
    set((s) => ({ shop: { ...s.shop, locked: !s.shop.locked } })),

  buyUnit: (shopIdx, slotIdx) => {
    const s = get();
    const unit = s.shop.units[shopIdx];
    if (!unit) return "Unit not available.";
    if (s.gold < UNIT_COST) return "Not enough gold.";
    const targetSlot = s.teamSlots[slotIdx];
    if (targetSlot.fini) return "Slot occupied. Sell first.";

    const newSlots: [TeamSlotState, TeamSlotState, TeamSlotState] = [
      ...s.teamSlots,
    ] as [TeamSlotState, TeamSlotState, TeamSlotState];
  ...
  buyItem: (shopIdx, slotIdx) => {
    ...
    if (!item) return "Item not available.";
    if (s.gold < item.cost) return "Not enough gold.";
    const target = s.teamSlots[slotIdx];
    if (!target.fini) return "Pick a Fini to equip this item on.";
  ...
  sellUnit: (slotIdx) =>
    set((s) => {
      const target = s.teamSlots[slotIdx];
      if (!target.fini) return s;
      const newSlots = [...s.teamSlots] as [TeamSlotState, TeamSlotState, TeamSlotState];
  ...
  swapTeamSlots: (a, b) =>
    set((s) => {
      const next = [...s.teamSlots] as [TeamSlotState, TeamSlotState, TeamSlotState];
      const tmp = next[a]!;
      next[a] = next[b]!;
      next[b] = tmp;
  ...
  equipItemOnSlot: (itemId, slotIdx) =>
    set((s) => {
      const next = [...s.teamSlots] as [TeamSlotState, TeamSlotState, TeamSlotState];
      if (!next[slotIdx].fini) return s;
  ...
  regenerateMarketSignals: () =>
    set((s) => ({ marketSignals: buildBattleSignals(s) })),
  ...
  refreshLiveMarket: async () => {
    ...
    try {
      const { signals, fetchedAt } = await fetchLiveMarketSignals(
        get().battleWindow,
      );
      ...
      set({
        marketLoading: false,
        marketError:
          err instanceof Error ? err.message : "Live market fetch failed.",
      });
    }
  },

  setDeathStake: (side, finiId) =>
    set((s) => {
      const teamPlayerId =
        side === "teamA" ? "player-a" : s.enemyTeam?.playerId ?? "cpu";
      return {
        deathMode: {
          ...s.deathMode,
          stakes: {
  ...
  startBattle: () => {
    ...
    if (!playerTeam) return "Your team is empty. Buy at least 1 Fini.";
    if (teamSlotsFilled(s.teamSlots) === 0) return "Your team is empty.";

    const enemy = s.enemyTeam;
    if (!enemy) return "No opponent.";
    ...
    if (s.mode === "DEATH") {
      ...
      if (!readiness.ok) return readiness.detail;
    }
    ...
    let events: BattleEvent[] = [...result.events];
    let finalResult: BattleResult = result;
    if (s.mode === "DEATH") {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!slotFini) continue;
      const updated = updatedRoster.find((f) => f.id === slotFini.id);
      if (updated) newSlots[i] = { ...newSlots[i]!, fini: updated };
    }
  ...
  advancePlayback: () => {
    const s = get();
    if (s.playbackStatus !== "playing") return;
...
function applyBattleOutcome(
  set: (fn: Partial<GameState> | ((s: GameState) => Partial<GameState>)) => void,
  get: () => GameState,
): void {
  const s = get();
  ...
  if (s.isRanked && s.currentOpponent) {
    ...
    for (const slot of s.teamSlots) {
      const f = slot.fini;
      if (!f || !f.id.startsWith("owned-")) continue;
