...
export interface FiniRecord {
  ...
  xp: number;
  level: number;
  lastBattleAt: number | null;   // epoch ms
  restingUntil: number | null;   // epoch ms — Fini cannot battle until this passes
  traitsEarned: string[];        // unlocked traits ("Diamond Body", etc.)
}
...
function restDuration(level: number): number {
...
interface RecordsState {
  records: Record<number, FiniRecord>;
  // Server-authoritative writes (in prod, these come from a /api/battle/resolve callback)
  applyBattleOutcome: (tokenId: number, outcome: "win" | "loss" | "draw") => void;
  applyBatch: (results: { tokenId: number; outcome: "win" | "loss" | "draw" }[]) => void;
  awardTrait: (tokenId: number, trait: string) => void;
  // Rest / potions
  restoreFully: (tokenId: number) => void;             // Energy Potion — fully clears cooldown
  shortenRest: (tokenId: number, ms: number) => void;  // Quick Snack — knocks off some time
  grantXp:     (tokenId: number, xp: number) => void;  // XP Truffle — direct XP bump
  // Read helpers
  get: (tokenId: number) => FiniRecord;
  isResting: (tokenId: number) => boolean;
  restingMsLeft: (tokenId: number) => number;
  // Server sync
  syncFromServer: (tokenIds: number[]) => Promise<void>;
  // Admin
  reset: (tokenId?: number) => void;
}

function emptyRecord(tokenId: number): FiniRecord {
  return { tokenId, wins: 0, losses: 0, draws: 0, xp: 0, level: 1, lastBattleAt: null, restingUntil: null, traitsEarned: [] };
}

export const useFiniRecords = create<RecordsState>()(
  persist(
    (set, getState) => ({
      records: {},
      ...
      shortenRest: (tokenId, ms) => {
        set(state => {
          const cur = state.records[tokenId] ?? emptyRecord(tokenId);
          if (!cur.restingUntil) return state;
      ...
      awardTrait: (tokenId, trait) => {
        set(state => {
          const cur = state.records[tokenId] ?? emptyRecord(tokenId);
          if (cur.traitsEarned.includes(trait)) return state;
      ...
      restingMsLeft: (tokenId) => {
        const r = getState().records[tokenId];
        if (!r?.restingUntil) return 0;
        return Math.max(0, r.restingUntil - Date.now());
      },

      reset: (tokenId) => {
        if (tokenId == null) { set({ records: {} }); return; }
        set(state => {
       ...
       * wallet connect.
       */
      syncFromServer: async (tokenIds: number[]) => {
        ...
        set(state => {
          ...
          for (const row of data) {
            next[row.token_id] = {
              ...
              traitsEarned: Array.isArray(row.traits_earned) ? row.traits_earned as string[] : [],
            };
          }
          return { records: next };
        });
...
// ── Team Power ───────────────────────────────────────────────────────────────
// A team's "Power Rating" combines level, XP, and battle history.
// This is the matchmaking number — opponents are paired within ±10% of yours.
//
//   power = sum over starters of:
//     level * 100                   ← base from progression
//     + (wins - losses) * 5         ← record bonus, can be negative
//     + (xp / 10)                   ← in-level progress
//
// A fresh team of 3 Lvl 1 Finis = ~300. A seasoned Lvl 5 trio at 80W/20L = ~2,000.

export interface TeamPower {
  total: number;
  byFini: { tokenId: number; power: number }[];
  tier: "Rookie" | "Bronze" | "Silver" | "Gold" | "Diamond" | "Legend";
  tierColor: string;
}

export function computeFiniPower(record: FiniRecord): number {
...
export function tierFor(power: number): { name: TeamPower["tier"]; color: string } {
  if (power >= 3000) return { name: "Legend",  color: "#dc2626" };
  if (power >= 2000) return { name: "Diamond", color: "#06b6d4" };
  if (power >= 1200) return { name: "Gold",    color: "#f59e0b" };
  if (power >= 700)  return { name: "Silver",  color: "#94a3b8" };
  if (power >= 400)  return { name: "Bronze",  color: "#a16207" };
  return                    { name: "Rookie",  color: "#84cc16" };
}

export function computeTeamPower(tokenIds: number[], getter: (id: number) => FiniRecord): TeamPower {
...
export function matchesPower(yourPower: number, theirPower: number, tolerance = 0.15): boolean {
  const diff = Math.abs(yourPower - theirPower) / yourPower;
  return diff <= tolerance;
}

export function fmtRestTime(ms: number): string {
